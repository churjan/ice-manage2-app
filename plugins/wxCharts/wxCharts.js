...
var config = {
	...
	colors: ['#60c0bd', '#eef2f7', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function drawRingTitle(opts, config, context) {
	...
	var bigtitlefontSize = opts.bigtitle.fontSize || config.subtitleFontSize;
    var title = opts.title.name || '';
    var subtitle = opts.subtitle.name || '';
	var bigtitle = opts.bigtitle.name || '';
    var titleFontColor = opts.title.color || config.titleColor;
	var subtitleFontColor = opts.subtitle.color || config.subtitleColor;
	var bigtitleFontColor = opts.bigtitle.color || config.bigtitleColor;
    var titleHeight = title ? titlefontSize : 0;
	var subtitleHeight = subtitle ? subtitlefontSize : 0;
	var bigtitleHeight = bigtitle ? bigtitlefontSize : 0;
    ...
    if (subtitle) {
        ...
        if (title) {
			startY = startY - (titleHeight + margin) / 2 - 20;
        }
        context.beginPath();
	...
	if (bigtitle) {
		var textWidth = measureText(bigtitle, bigtitlefontSize);
		var startX = (opts.width - textWidth) / 2 + (opts.bigtitle.offsetX || 0);
		var startY = (opts.height - config.legendHeight + bigtitlefontSize) / 2;
		context.beginPath();
		context.setFontSize(bigtitlefontSize);
		context.setFillStyle(bigtitleFontColor);
		context.fillText(bigtitle, startX, startY);
		context.stroke();
		context.closePath();
	}
    if (title) {
        ...
        if (subtitle) {
			_startY = _startY + (subtitleHeight + margin) / 2 + 20;
        }
        context.beginPath();
...
function drawPointText(points, series, config, context) {
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            ...
            context.fillText(formatVal, item.x - measureText(formatVal)/2 - 10, item.y - 4);
        }
    });
    context.closePath();
    context.stroke();
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        // 区域
		if (seriesIndex == 0) {
			splitPointList.forEach(function (points, i) {
        ...
        splitPointList.forEach(function (points, index) {
          context.beginPath();
          // context.setStrokeStyle(eachSeries.color);
          context.setStrokeStyle(eachSeries.pointColor);
          context.setLineWidth(2);
          if (points.length === 1) {
        ...
        if (opts.dataPointShape !== false) {
            // var shape = config.dataPointShape[seriesIndex % config.dataPointShape.length];circle
            // drawPointShape(points, eachSeries.color, shape, context);
            drawPointShape(points, eachSeries.pointColor, 'circle', context);
        }
    });
    if (opts.dataLabel !== false && process === 1) {
        series.forEach(function (eachSeries, seriesIndex) {
...
function drawPieDataPoints(series, opts, config, context) {
	...
	// console.log(opts.padding,config.padding,11)
    var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    if (opts.type === 'ring') {
		// console.log(config.padding,1111)
